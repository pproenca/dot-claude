...
# Find branch point
branch_point="$(git merge-base HEAD "origin/${main_branch}" 2>/dev/null || echo "")"

if [[ -z "${branch_point}" ]]; then
    escaped_main="$(printf '%s' "${main_branch}" | sed 's/\\/\\\\/g' | sed 's/"/\\"/g')"
...
branch_point_msg="$(git log -1 --format=%s "${branch_point}" 2>/dev/null || echo "")"
...
for file in "${files[@]}"; do
    ...
    stat_line="$(git diff --numstat "${branch_point}" HEAD -- "${file}" 2>/dev/null | head -1)"
    add="$(echo "${stat_line}" | awk '{print $1}')"
    ...
    stats_json+="{\"file\":\"${escaped_file}\",\"additions\":${add},\"deletions\":${del}}"
done
stats_json+="]"
...
for file in "${files[@]}"; do
    ...
    file_diff="$(git diff "${branch_point}" HEAD -- "${file}" 2>/dev/null || echo "")"
...
escaped_msg="$(printf '%s' "${branch_point_msg}" | sed 's/\\/\\\\/g' | sed 's/"/\\"/g')"
