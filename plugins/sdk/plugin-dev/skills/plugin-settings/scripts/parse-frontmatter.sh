...
# Escape field name for safe sed usage (prevents injection)
escape_sed() {
  printf '%s\n' "$1" | sed -e 's/[\/&]/\\&/g' -e 's/[.^$*+?()[{|\\]/\\&/g'
}
FIELD_ESCAPED=$(escape_sed "$FIELD")

# Extract specific field (using escaped field name for security)
VALUE=$(echo "$FRONTMATTER" | grep "^${FIELD}:" | sed "s/${FIELD_ESCAPED}: *//" | sed 's/^"\(.*\)"$/\1/' | sed "s/^'\\(.*\\)'$/\\1/")
