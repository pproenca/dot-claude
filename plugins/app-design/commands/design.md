...
1. Read `.claude/app-design.local.md` if it exists
...
> - [x/blank] Features organized into sections (`design/features.md` → Sections)
...
Ask these questions one at a time using AskUserQuestion, skipping any already answered in `.local.md`:
...
Write `design/goals.md` and create/update `.claude/app-design.local.md`.
...
5. **Group into sections** — Organize essential features into logical sections (clusters of related features from the user's perspective). Each section gets a name, purpose, features, and user value.

Write `design/features.md`. Update `.local.md` to set `current_phase: "architect"`.
...
Suggest next steps:
- "Run `/app-design:validate` to audit your code against the design specs and find gaps"
