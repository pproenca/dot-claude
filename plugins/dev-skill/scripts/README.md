...
# Validate sections only (for early fail-fast during generation)
node validate-skill.js ./skills/react-best-practices --sections-only

# Verify AGENTS.md matches script output (detect manual edits or stale file)
...
The `--verify-generated` flag ensures AGENTS.md was generated by the script and not manually edited.

### Why Verify?

- **Consistency**: Manual edits to AGENTS.md will be lost when the script runs
- **Detect staleness**: If rules are updated but AGENTS.md wasn't regenerated
- **CI/CD integration**: Catch accidental manual edits in pull requests

### Usage

```bash
# Check if AGENTS.md matches script output
node validate-skill.js ./skills/react-best-practices --verify-generated

# Combine with other validation
node validate-skill.js ./skills/react-best-practices --strict --verify-generated
```
...
| Error | Cause | Fix |
|-------|-------|-----|
| `AGENTS.md not found` | File doesn't exist | Run `node scripts/build-agents-md.js <skill-dir>` |
| `AGENTS.md differs from script output` | Manual edits or stale file | Regenerate with `node scripts/build-agents-md.js <skill-dir>` |
