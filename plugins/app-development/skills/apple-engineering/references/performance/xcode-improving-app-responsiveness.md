...
[image: An illustration that depicts how an app uses an asynchronous dispatch queue to increase the responsiveness of its main thread.]
...
To learn more about different types of hitches and the stages of the render loop, see [Understanding hitches in your app](https://developer.apple.com/documentation/xcode/understanding-hitches-in-your-app).
...
> **Note:** - See [Optimizing iPhone and iPad apps to support ProMotion displays](https://developer.apple.com/documentation/QuartzCore/optimizing-iphone-and-ipad-apps-to-support-promotion-displays) to learn more about working with variable refresh rates. - See [Optimize for variable refresh rate displays](https://developer.apple.comhttps://developer.apple.com/videos/play/wwdc2021/10147/) to learn about the difference between fixed-rate and adaptive-sync displays, and how to make the most of variable refresh rate displays.
...
- Turn on the Thread Performance Checker in your app’s scheme to receive notifications of priority inversions when you run your app from Xcode. Learn more at [Diagnosing performance issues early](https://developer.apple.com/documentation/xcode/diagnosing-performance-issues-early).
...
[image: An illustration with two histogram charts side by side as they also appear in Instruments in the Time Profiler instruments CPU Usage track. The overall illustration has the title, Unresponsive Main Threads. The left chart shows a graph where the bars reach to the top, or close to the top, for most of the width of the chart. It has the caption, busy main thread. In the chart on the right, the first portion looks similar, but there is a large gap where no bars appear for the majority of the width of the chart. Toward the end, the bars show up again. This chart has the caption, blocked main thread.]
...
[image: A screenshot of Instruments showing a recorded trace from the Hitches template. Under the track for the Hitches instrument, there are tracks for User Events, Commits, Renders, GPU, and Frame Lifetimes. There is also an additional track showing how long individual frames are on the display and when the vsyncs happen. The tracks show the components of a single hitch in the current time interval, and there are six hitches overall in the list below the track area.]
