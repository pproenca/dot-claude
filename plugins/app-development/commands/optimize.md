---
description: Profile and optimize app performance using Apple's 8 key metrics
...
5. Read `${CLAUDE_PLUGIN_ROOT}/skills/apple-engineering/SKILL.md` for the reference structure
...
Then ask for confirmation rather than open-ended questions using AskUserQuestion:
...
Ask these questions using AskUserQuestion, skipping any already answered:
...
Read `${CLAUDE_PLUGIN_ROOT}/skills/apple-engineering/indexes/performance-by-metric.md`.

For EACH of Apple's 8 key performance metrics, read the relevant reference doc from `${CLAUDE_PLUGIN_ROOT}/skills/apple-engineering/references/performance/` and assess the app:
...
Read the MetricKit references from `${CLAUDE_PLUGIN_ROOT}/skills/apple-engineering/references/performance/`.
