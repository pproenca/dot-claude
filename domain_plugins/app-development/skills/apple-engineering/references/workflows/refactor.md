...
4. If `engineering/requirements.md` or `engineering/persistence.md` are missing, tell the user to run `modeling workflow` first — STOP here
...
6. Read `SKILL.md` for the reference structure
...
Read `indexes/data-by-need.md`.

Read existing `engineering/persistence.md`. Compare the current strategy against what Apple's reference docs recommend for the (possibly updated) requirements.

Read the FULL reference from `references/data/` for each candidate.
