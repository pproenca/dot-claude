---
description: Full App development workflow — check progress and continue from where you left off
...
1. Read `.claude/app-development.local.md` if it exists
...
Write `engineering/requirements.md` and create/update `.claude/app-development.local.md`.

#### Step 1: Choose Persistence Strategy

Read `${CLAUDE_PLUGIN_ROOT}/skills/apple-engineering/indexes/data-by-need.md` and relevant reference docs. Evaluate SwiftData, Core Data, and alternatives. Present with Apple quotes. Save to `engineering/persistence.md`.
...
Tell the user: "Data model is complete. Running the performance phase. Use `/app-development:optimize` for the full process."
...
Suggest next steps:
- "Run `/app-development:optimize` to re-audit performance"
- "Run `/app-development:debug` when you hit an issue"
- "Run `/app-development:refactor` to rework the data model"
