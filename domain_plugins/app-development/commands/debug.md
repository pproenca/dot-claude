---
description: Diagnose and fix issues using Apple's debugging tools — LLDB, Instruments, crash logs
...
4. Read `${CLAUDE_PLUGIN_ROOT}/skills/apple-engineering/SKILL.md` for the reference structure
...
Read the relevant reference docs from `${CLAUDE_PLUGIN_ROOT}/skills/apple-engineering/references/performance/` and `references/swift/`.
...
Use AskUserQuestion to collect findings:
...
Read the relevant Swift references from `${CLAUDE_PLUGIN_ROOT}/skills/apple-engineering/references/swift/` for prevention patterns.
